...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    analogWrite(PIN_LED,(200.0-dist_raw)*255/100);
  }
  else {
    analogWrite(PIN_LED,(dist_raw-200.0)*255/100);
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = B; // return 0 when out of range.
  return reading;
